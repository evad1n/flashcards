...
getEntries(TABLE_NAME, refreshData)
...
function deleteEntries(num) {
    ...
    for (var i = 1; i <= num; i++) {
        ...
        var deletedId = list[list.length - i].Id;
        console.log(idLookUp(deletedId));
        deleteEntry(TABLE_NAME, deletedId)
        //refresh table
        populateTable();
...
function autocomplete(inp) {
    ...
    inp.addEventListener("input", function (e) {
        ...
        for (i = 0; i < list.length; i++) {
            ...
            if (list[i].Word.substr(0, val.length).toUpperCase() == val.toUpperCase()) {
                /*create a DIV element for each matching element:*/
                b = document.createElement("DIV");
                /*make the matching letters bold:*/
                b.innerHTML = "<strong>" + list[i].Word.substr(0, val.length) + "</strong>";
                b.innerHTML += list[i].Word.substr(val.length);

                //display the part of speech after the word
                b.innerHTML += (" (" + list[i].PartOfSpeech + ")");

                /*insert a input field that will hold the current array item's value:*/
                b.innerHTML += "<input type='hidden' value='" + list[i].Word + "'>";
