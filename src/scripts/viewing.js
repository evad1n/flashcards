...
let started = false;
let list, history, index;
let frequencies;

openDatabase()

localStorage = window.localStorage;
const TABLE_NAME = JSON.parse(localStorage.getItem("table-name"));

// const TABLE_NAME = getTableName()

// function getTableName() {
//     url = window.location.href
//     index = url.indexOf("table")
//     tableName = url.substring(index + 6, url.length)
//     return tableName
// }
...
//set up data
getEntries(TABLE_NAME, setDictionary)
...
function setDictionary(data) {
    console.log("length of word list: " + Object.keys(list).length);
    ...
    if (frequencies != null) {
        ...
        Object.keys(list).length.forEach(key => {
            frequencies[key] = DEFAULT_FREQUENCY
        });
...
function autocomplete(inp) {
    ...
    inp.addEventListener("input", function (e) {
        ...
        Object.keys(list).forEach(key => {
            ...
            if (list[key].Word.substr(0, val.length).toUpperCase() == val.toUpperCase()) {
                /*create a DIV element for each matching element:*/
                b = document.createElement("DIV");
                /*make the matching letters bold:*/
                b.innerHTML = "<strong>" + list[key].Word.substr(0, val.length) + "</strong>";
                b.innerHTML += list[key].Word.substr(val.length);

                //display the part of speech after the word
                b.innerHTML += (" (" + list[key].PartOfSpeech + ")");

                /*insert a input field that will hold the current array item's value:*/
                b.innerHTML += "<input type='hidden' value='" + list[key].Word + "'>";
